...
function CategoryCard({ CategoryData , className , overlay}: CatagotyProps) {
  ...
  return (
    <Card className={cn('group  cursor-target  font-serif! cursor-target   rounded-xl overflow-hidden bg-background  p-0 min-h-[4em] shadow-none border-0  w-full  relative ' ,

       className
    )}>
<Link href={`/products/category/${CategoryData.value}`}>
{overlay ? (
