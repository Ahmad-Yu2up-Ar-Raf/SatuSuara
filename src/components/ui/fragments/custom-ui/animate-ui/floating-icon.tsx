  ...
  return (
   ...
   viewport={{ once: true }}
    ...
    >
      ...
      <motion.div
        className="flex items-center justify-center size-11 md:size-15 p-2 md:p-3 rounded-3xl shadow-xl bg-card/80 backdrop-blur-md border border-border/10"
        animate={{
          y: [0, -8, 0, 8, 0],
          x: [0, 6, 0, -6, 0],
          rotate: [0, 5, 0, -5, 0],
      ...
      >
        <iconData.icon className="size-10 md:w-10 md:h-10 text-foreground" />
      </motion.div>
    </motion.div>
  );
};
...
>(({ className, title, subtitle, ctaText, ctaHref, icons, ...props }, ref) => {
  ...
  return (
    <section
      ...
      className={cn(
        'relative w-full min-h-[70lvh]   md:min-h-lvh    flex items-center justify-center overflow-visible bg-background',
        className
      )}
      {...props}
    >
      ...
      {/* Container for the foreground content */}
        ...
        <TextAnimate once duration={2} as={"p"}  className=" relative max-w-xs  text-center  xl:text-2xl md:max-w-xl mx-auto sm:text-lg lg:text-xl text-muted-foreground">
          {subtitle}
        </TextAnimate>
